...
import validateManyFields from "../validations/validation";
...
const SignupForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setFormErrors({});
    //if any error, populate from error state and stop furthur excecution
    if (errors.length > 0) {
      setFormErrors(
    ...
    fetchData(config)
      ...
      .catch((err) => {
        console.log(`Error occured during API calls: `, err.response.data.msg);
      });
  };
      ...
      <form className="m-auto my-16 max-w-[500px] p-8 bg-white border-2 shadow-md rounded-md">
        {loading ? (
          ...
          <>
            ...
            <button
              className="bg-green-800 text-white px-4 py-2 font-medium hover:bg-primary-dark"
